...
export const createCinemas = (image, newCinema) => async dispatch => {
  try {
    ...
    const cinema = await response.json();
    if (response.ok) {
      dispatch(setAlert('la salle de cinema esy creer avec suuces', 'success', 5000));
      if (image) dispatch(uploadCinemaImage(cinema._id, image));
      dispatch(getCinemas());
      return { status: 'success', message: 'la salle de cinema a ete cree' };
    }
  } catch (error) {
    dispatch(setAlert(error.message, 'error', 5000));
    return {
      status: 'error',
      message: ' la salle de Cinema est n est pas sauvgarder. essayer ulterieurement'
    };
  }
};
...
export const updateCinemas = (image, cinema, id) => async dispatch => {
  try {
    ...
    return {
      status: 'error',
      message: ' la salle de cinema n est pas mis a jour .. essayer ulterieurement'
    };
  }
};
...
export const removeCinemas = id => async dispatch => {
  try {
    ...
    return {
      status: 'error',
      message: ' la salle de cinema n est pas supprimer, essayer ulterieurement.'
    };
  }
};
