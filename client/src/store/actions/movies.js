...
export const uploadMovieImage = (id, image) => async dispatch => {
  try {
    ...
    if (response.ok) {
      dispatch(setAlert('Image Sauvgarder', 'success', 5000));
    }
...
export const addMovie = (image, newMovie) => async dispatch => {
  try {
    ...
    const movie = await response.json();
    if (response.ok) {
      dispatch(setAlert('le film a ete sauvgarder !', 'success', 5000));
...
export const removeMovie = movieId => async dispatch => {
  try {
    ...
    if (response.ok) {
      dispatch(getMovies());
      dispatch(onSelectMovie(null));
      dispatch(setAlert('le film a ete supprimer', 'success', 5000));
    }
  } catch (error) {
    dispatch(setAlert(error.message, 'error', 5000));
  }
